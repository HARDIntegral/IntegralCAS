...
void parse(char *input) {
  ...
  // Prepare Shunting-Yard by cleaning the tokens
  ...
  LIST *operators = generateList(); // Will be a queue
  LIST *numbers = generateList();   // Will be a stack

  void *parsed_data = NULL;
  TYPE *var_type = NULL;

  // From this point on until the next segment, just know that I was brain ded
  while (token_intermediate->data_list->size > 0) {
    printf("break\n");
    popParseList(token_intermediate, parsed_data, var_type);
    if (*var_type == INT) {
      if (push(numbers, parsed_data) == FAILURE)
        printf("Failure like me\n");
    } else if (*var_type == OPER) {
      if (append(operators, parsed_data) == FAILURE)
        printf("Another failure like me\n");
    }
    printf("test");
    
    iterate(numbers, printInteger, 0);
    iterate(operators, printOperator, 0);
    printf("size left: %d\n", token_intermediate->data_list->size);
  }
  ...
  if (destroyParseList(token_intermediate) == FAILURE)
    printf("FAILURE DEALLOCATING LIST");
  if (destroyList(operators) == FAILURE)
    printf("FAILURE DEALLOCATING LIST");
  if (destroyList(numbers) == FAILURE)
    printf("FAILURE DEALLOCATING LIST");
  ...
  while (tmp_token != NULL) {
    ...
    CATCH(NOTANUMBEREXCEPTION) {
      ...
      if (strcmp((char *)data, "left") == 0)
        oper_capacity[counter] = LEFT;
      if (strcmp((char *)data, "right") == 0)
        oper_capacity[counter] = RIGHT;
